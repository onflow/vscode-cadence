...
get_architecture() {
    ...
    case "$_cputype" in
        x86_64 | x86-64 | x64 | amd64)
            ...
            ;;
        aarch64)
            _cputype=arm64 # workaround for arm cpu

            if [ "$_ostype" = linux ]; then # special case where we run docker with linux on osx arm
              _targetpath=$HOME/.local/bin
              _ostype=linux
              echo "PATH=$_targetpath:$PATH" >> ~/.bashrc # workaround for adding bin path
            else
              _ostype=darwin
            fi
            ;;
