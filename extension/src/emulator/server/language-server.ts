...
import { CodeLensParams, CodeLensRequest, ExecuteCommandRequest, TextDocumentIdentifier, CodeLens } from 'vscode-languageclient'
...
export class LanguageServerAPI {
  ...
  async refreshCodeLens (): Promise<CodeLens[] | null | undefined> {
    const textDocumentUri = window.activeTextEditor?.document.uri
    if (textDocumentUri == null) return
    
    return await this.client?.sendRequest(
      CodeLensRequest.type,
      <CodeLensParams>{ textDocument: TextDocumentIdentifier.create(textDocumentUri.toString()) }
    )
  }
