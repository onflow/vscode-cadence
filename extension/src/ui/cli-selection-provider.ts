...
import { CliBinary, CliProvider } from "../flow-cli/cli-provider";
import { SemVer } from "semver";

const TOGGLE_CADENCE_VERSION_COMMAND = "cadence.changeCadenceVersion";
const CADENCE_V1_CLI_REGEX = /-cadence-v1.0.0/g;
const GET_BINARY_LABEL = (binary: CliBinary): string => `Flow CLI ${binary.version}`;
...
export class CliSelectionProvider implements vscode.Disposable {
    ...
    constructor(cliProvider: CliProvider) {
        ...
        vscode.commands.registerCommand(TOGGLE_CADENCE_VERSION_COMMAND, () => this.#toggleSelector(true));
    ...
    #createStatusBarItem(version: SemVer): vscode.StatusBarItem {
        ...
        statusBarItem.text = `Flow CLI v${version}`;
    ...
    #createVersionSelector(currentBinary: CliBinary | null, availableBinaries: CliBinary[]): vscode.QuickPick<AvailableBinaryItem | CustomBinaryItem> {
        ...
        this.#disposables.push(versionSelector.onDidAccept(() => {
        ...
        const items: (AvailableBinaryItem | CustomBinaryItem)[] = availableBinaries.map(binary => new AvailableBinaryItem(binary));
    ...
    async #toggleSelector(show: boolean) {
...
class AvailableBinaryItem implements vscode.QuickPickItem {
    ...
    get label(): string {
        return GET_BINARY_LABEL(this.#binary);
    }
