...
function expectTreeToEqual(testCollection: vscode.TestItemCollection, expected: TreeResult[]) {
  function buildActualTree(testCollection: vscode.TestItemCollection): TreeResult[] {
    let items: TreeResult[] = []
...
suite('test trie tests', () => {
  ...
  test('adds and removes files from test tree', () => {
    ...
    expectTreeToEqual(testController.items, [{
      id: vscode.workspace.workspaceFolders![0].uri.fsPath,
      label: 'workspace',
      tags: ['cadence'],
      uri: vscode.workspace.workspaceFolders![0].uri.fsPath,
      children: [
        {
          id: 'test1.cdc',
          label: 'test1.cdc',
